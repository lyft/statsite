...
START_TEST(test_metrics_histogram)
{
    ...
    // Invalid metric values result in timer values not converging
    // while computing quantiles, checks for them!
    fail_unless(metrics_add_sample(&m, TIMER, "foo",  sqrt(-1), 1.0 ) == -1);
    fail_unless(metrics_add_sample(&m, TIMER, "foo",  (-1.0 / 0.0), 1.0 ) == -1);
    fail_unless(metrics_add_sample(&m, TIMER, "foo",  (1.0 / 0.0), 1.0 ) == -1);
